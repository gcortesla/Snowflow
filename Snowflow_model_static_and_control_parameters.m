...
params.K_SW     = 1;      % Soil water (rootzone) conductivity (m/day)
params.K_DW     = 1;      % Deep water conductivity (m/day)
params.f        = 0.3;           % prefered flow direcion (1 = horz., 0 = ver.) (-)
params.Kc       = 1.1;          % Crop coef. (-)
params.SW       = 1;           % Soil water capacity (m)
params.DW       = 3;            % Deep water capacity (m)
params.RRF      = 1;        % Runoff resistance factor (-)
...
params.RFsnow       = 0.0160;    % 0.00052 * 24 m^2mm/day/K
params.RFice        = 0.0250;    % 0.00106 m^2mm/day/K
...
params.T_icemelt    = 278;   % Icemelt temp. (K)

params.T_S          = 278;           % Freezing point (K)
params.T_L          = 280;           % Melting point (K) 
