...
for n_sim = 1:n_iter;
    ...
    %% Run the snow model(DDM)function only if snow is on the ground or falling
            ...
            if SWE_old(n)>0 || P_snow(n,tt)>0 
                ...
                melt_ice(n, tt)     = (A_glacier(n)/A_band(n)).*melt_ice(n, tt); 
    ...
    %% Call Z1 update over non-glacierized part of the elevation band (Bucket #1)

            [Z1(n,tt), Surface_runoff(n,tt), Infiltration(n,tt), Interflow(n,tt), Percolation(n,tt)] = ...
                             Z1_update(Z1_old(n),params.f,...
                             params.K_SW,params.RRF,params.k,...
                             P_rain(n,tt),melt_snow(n,tt), melt_ice(n,tt), params.SW);

    %% Call Z2 update over non-glacierized part of the elevation band (Bucket #2)

            [Z2(n,tt), Baseflow(n,tt)] = Z2_update(Z2_old(n), Percolation(n, tt), params.K_DW, params.DW);

    %% Calculate total flow volume in m^3
            %Surface_runoff(n, tt) = Surface_runoff
        ...
        if mod(tt, control_params.n_year) == 0
            %% Overall change in the volume of glacial ice within the watershed
            
            delta_V_glacier(tt/control_params.n_year) = nansum(delta_V_ice(:, tt/control_params.n_year)); % (m^3)
            
            %GC add SWE from last year to glacier
            delta_V_glacier(tt/control_params.n_year) = delta_V_glacier(tt/control_params.n_year);
            %GC end mod
            
            %% Annual glacier geometry evolution

            A_glacier_init = sum(A_glacier(:));
            V_glacier_init = params.glacier_c.*(A_glacier_init)^(params.glacier_b);

            V_glacier_final(tt/control_params.n_year) = V_glacier_init + delta_V_glacier(tt/control_params.n_year);
            
            if V_glacier_final(tt/control_params.n_year) < 0
                V_glacier_final(tt/control_params.n_year) = 0;
            end;
            
            A_glacier_final(tt/control_params.n_year) = ((V_glacier_final(tt/control_params.n_year))/params.glacier_c)^(1/params.glacier_b);
            ...
            %bands_with_glaciers = find(A_glacier>0);
            
            if V_glacier_final>0
                for n = bands_with_glaciers;
                    A_glacier_band(n, tt/control_params.n_year) = A_glacier(n) + weights(n)*dummy_delta./nansum(weights(bands_with_glaciers));
...
if control_params.Monte_Carlo == 0
    ...
    eval(['save ' control_params.output_filename ' control_params date_data Outflow' ' V_glacier_final' ' A_glacier_final' ' delta_V_glacier' ' delta_A_glacier' ...
        ...
        ' Interflow' ' Baseflow' ' Infiltration' ' Percolation' ' mc' ' Ta' ' PPT' ' Mpot_snow' ' Mpot_ice' ' A_glacier_band' ' A_non_glacier_band' ' Streamflow' ' SWE_reanalysis' ' weights']) 
    ...
    subplot(2, 2, 2);
    plot(date_data, squeeze(nansum(SWE_model, 2)),  'LineStyle', '-', 'Color', [.4 .4 .4], 'LineWidth', 2);
    hold on;
    datetick;
    
    if control_params.validation_flag_SWE == 1
        plot(date_data, nansum(SWE_reanalysis, 1), 'r');
    end;
