function Snowflow_model_driver(basin, run_sce, scenario, years, flag_SWE, flag_Q, precip_scaling)
    ...
    for tt = 1:control_params.nt
        ...
        for n = 1:length(elev_bands)
            ...
            [Z1(n,tt), Surface_runoff(n,tt), Infiltration(n,tt), Interflow(n,tt), Percolation(n,tt), ET(n, tt)] = ...
                ...
                P_rain(n,tt),melt_snow(n,tt), melt_ice(n, tt), params.SW, PET(mod(tt, 366)+1));
            
            %GC replace melt_ice to go into surface runoff instead of to Z1
           % Surface_runoff(n, tt) = Surface_runoff(n, tt) + melt_ice(n,tt);
...
if control_params.Monte_Carlo == 0
    ...
    eval(['save ' control_params.output_filename ' control_params date_data Outflow' ' V_glacier_final' ' A_glacier_final' ' delta_V_glacier' ' delta_A_glacier' ...
        ...
        ' Interflow' ' Baseflow' ' Infiltration' ' Percolation' ' mc' ' Ta' ' PPT' ' Mpot_snow' ' Mpot_ice' ' A_glacier_band' ' A_non_glacier_band' ' Streamflow' ' SWE_reanalysis' ' weights' ' ET'])
